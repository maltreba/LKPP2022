        ...
        (  SELECT kode_detail,
             ...
             FROM (SELECT *
                     FROM (SELECT    bel_kode_kementerian
                                  ...
                                  MAX (bel_revisi_ke)
                                      OVER (PARTITION BY /*
     bel_kode_kementerian||'.'||bel_kode_unit||'.'||out_kode_program||'.'||out_kode_kegiatan||'.'||out_kode_output||'.'||subout_kode,
     bel_kode_satker,*/
                                                         BEL_NOMOR_DIPA)    max_rev
                             FROM SAKTIRKA.MV_ANGGARAN_BASE_TABLE_230930_231123)
                    WHERE BEL_REVISI_KE = max_rev AND item_harga_satuan != 0 --Tambahan 12 Desember 2023
                                                                            )
